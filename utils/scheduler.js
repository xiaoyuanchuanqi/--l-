...
moment.locale('zh-cn');
const {getCookies, delCookiesFile} = require('./util')
...
let tasks = {}
let scheduler = {
  taskFile: path.join(path.resolve('./'), '.AutoSignMachine', 'taskFile.json'),
  today: '',
  isRunning: false,
  isTryRun: false,
  taskJson: undefined,
  ...
  buildQueues: async (taskNames, queues) => {
    for (let taskName of taskNames) {
      ...
      // 任务的随机延迟时间
      let waitTime = Math.floor(Math.random() * (20))
      if (options) {
      ...
      willTime = willTime.subtract(1, 'days')
      queues.push({
        taskName: taskName,
        taskState: 0,
